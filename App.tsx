...
const App: React.FC = () => {
  ...
  const [mainView, setMainView] = useState<'home' | 'projects' | 'workspace' | 'templates' | 'knowledge' | 'agents' | 'settings'>('home');
            ...
            <div className="flex p-1 bg-surface border border-border rounded-lg">
                <button 
                    ...
                    className={`flex items-center gap-2 px-3 py-1.5 text-xs font-medium rounded-md transition-all ${
                        activeView === 'preview' ? 'bg-primary text-white shadow-sm' : 'text-gray-400 hover:text-white'
                    }`}
                >
                    <Eye size={14} /> Preview
                </button>
                <button 
                    ...
                    className={`flex items-center gap-2 px-3 py-1.5 text-xs font-medium rounded-md transition-all ${
                        activeView === 'code' ? 'bg-primary text-white shadow-sm' : 'text-gray-400 hover:text-white'
                    }`}
                >
                            ...
                            <div key={proj.id} className="group bg-surface border border-border rounded-xl p-5 hover:border-primary/50 hover:shadow-lg transition-all duration-300 flex flex-col h-full relative overflow-hidden">
                                ...
                                <div className="mt-auto flex items-center justify-between pt-4 border-t border-border/50">
                                    ...
                                    <span className={`text-[10px] px-2 py-0.5 rounded-full border ${
                                        proj.status === 'Ready' || proj.status === 'Live' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 
                    ...
                    <PanelGroup direction="horizontal">
                        ...
                        {isTerminalOpen && (
                            <>
                                ...
                                <PanelResizeHandle className="w-1 bg-border hover:bg-primary transition-colors cursor-col-resize flex items-center justify-center group z-10">
